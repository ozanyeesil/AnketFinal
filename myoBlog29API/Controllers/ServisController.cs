...
namespace myoBlog29API.Controllers
{
    ...
    public class ServisController : ApiController
    {
        ...
        {
            if (db.Cevap.Count(s => s.UyeId == model.UyeId) > 0)
            {
                sonuc.islem = false;
                sonuc.mesaj = "Zaten ankete oy verilmiş!";
                return sonuc;
            }
        ...
        [HttpGet]
        [Route("api/cevap1/{makaleId}")]
        public CevapModel Cevap1Liste(int makaleId)
        {
            CevapModel kayit = db.Cevap.Where(s => s.CevapId == makaleId && s.Cevap1 == 1).Select(x => new CevapModel()
            {
                MakaleId = x.MakaleId,
                Cevap1 = x.Cevap1,
                UyeId = (int)x.UyeId,
        ...
        [HttpGet]
        [Route("api/cevap2/{makaleId}")]
        public List<CevapModel> Cevap2Liste(int makaleId)
        {
            List<CevapModel> liste = db.Cevap.Where(s => s.MakaleId == makaleId && s.Cevap1 == 2).Select(x => new CevapModel()
            {
                MakaleId = x.MakaleId,
                Cevap1 = x.Cevap1,
                UyeId = (int)x.UyeId,
        ...
        [HttpGet]
        [Route("api/cevap3/{makaleId}")]
        public List<CevapModel> Cevap3Liste(int makaleId)
        {
            List<CevapModel> liste = db.Cevap.Where(s => s.MakaleId == makaleId && s.Cevap1 == 3).Select(x => new CevapModel()
            {
                MakaleId = x.MakaleId,
                Cevap1 = x.Cevap1,
                UyeId = (int)x.UyeId,
